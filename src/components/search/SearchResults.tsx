    ...
    <div className="space-y-6">
      <div className="grid gap-6">
        {results.items.map((item, index) => {
          const innerClasses = "group bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300";

          if (item.type === 'mentor') {
            return (
              <Link 
                key={`${item.type}-${index}`} 
                to={`/mentors/${item.data.id}`}
                className={`block ${innerClasses}`}
              >
